...
<head>
  ...
  <script>
    ...
    function updateTotal() {
      ...
      totalElement.textContent = total.toFixed(2) + " ریال";
    }
  </script>
</head>
<body>
     ...
     <% sno=sno+1; %>
      <td class="right-align">  <% out.print(sno);%> </td>
        <td><img src="./img/machin7.jpg" alt="تصویر محصول" class="product-image"></td>
          <td name="name" class="right-align"><%=rs.getString(4)%> <br><%=rs.getString(5)%></td>
        
       
         <td name="name" class="right-align"><%=rs.getString(1)%></td>
            <td name="category" class="right-align"><%=rs.getString(2) %></td>
           <td name="quantity"><a href="IncDecQuantityAction.jsp?product_ID=<%=rs.getString(10)%>&quantity=increased"><i class='fas fa-plus-circle'> </i></a> <%=rs.getString(13) %> <a href="IncDecQuantityAction.jsp?product_ID=<%=rs.getString(10)%>&quantity=decreased"><i class="fas fa-minus-circle"></i></a> </td> 
        <td name="price"> <%=rs.getString(3) %></td>
        <td name="total">  <%=rs.getString(15) %></td>
        <td ><a  href="RemoveFromCart.jsp?product_ID=<%=rs.getString(10) %>"> Remove <i class='fas fa-trash-alt'> </i> </a>  </td>
      </tr>
      <%} %>
